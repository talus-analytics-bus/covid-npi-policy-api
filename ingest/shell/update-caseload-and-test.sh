...
bash sh/update-metric-prod-from-local.sh $username $dbmetriclocal $dbprodhost && \  
bash ingest/sql/ingest.sh $username $dblocal && \
bash sh/update-test-from-local.sh $username $dblocal $dbprodhost && \
bash sh/start.sh && \
echo Started API server, please update IDEA Landing site and production COVID AMP database next.;